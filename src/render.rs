...
pub fn render_backgound(background: &Background) {
  ...
  let mut texture = Texture::from_image(&mut texture_context, &canvas, &TextureSettings::new()).unwrap();
  ...
  for bg in &background.tiles {
    for line in 0..8 {
      let mut liney = 0;
      for pixels in 0..8 {
        let pixel = bg.sprite[line][pixels];
        linex += 1;
        if linex >= 256 {
          linex = 0;
          liney += 1;
        ...
        let color_id = bg.plallet[pixel as usize];
        let red = colors[color_id as usize].0;
        let green = colors[color_id as usize].1;
        let blue = colors[color_id as usize].2;

        canvas.put_pixel(linex, liney, im::Rgba([red, green, blue, 0]));
      }
    }
  }
